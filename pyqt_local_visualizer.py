...
print('load_data')
path = '/home/tim/Work/Simulations/TVB/'
...
global tavgs
#print os.getcwd()
tavgs = np.load('../TVB/my_tvb_data/simulations/TAVG_5.npy')
...
seegs_not_filtered = np.load('../TVB/my_tvb_data/simulations/SEEG_5.npy')
...
# second window
pw2 = pg.PlotWidget(name='Plot2', title='TAVG_3')
l.addWidget(pw2, 1, 0)
pw2.setXLink(pw1)
...
pw3 = gl.GLViewWidget()
l.addWidget(pw3, 0, 1)
pw3.setCameraPosition(distance=200, azimuth=-210)
...
surf_item = gl.GLMeshItem(vertexes=verts[:], faces=faces[:],
                  drawFaces=True, drawEdges=False, color=(32, 32, 32, 0.5),
...
for i in range(seegs.shape[0]):
    ...
    seeg_item[i].translate(-positions[i,0], -positions[i,1], -positions[i,2] )
...
for i in range(centres.shape[0]):
    ...
    centres_item1[i].translate(centres[i,0], centres[i,1] , centres[i,2])
...
pw4.setCameraPosition(distance=200, azimuth=-210)
...
for i in range(centres.shape[0]):
    ...
    centres_item[i].translate(centres[i,0], centres[i,1] , centres[i,2])
